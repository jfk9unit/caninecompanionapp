...
export const DailyActivities = ({ user }) => {
  ...
  const handleAddTask = async (dogId) => {
    if (!newTask.title) {
    ...
    try {
      ...
      fetchTasks(dogId, dateStr);
    } catch (error) {
      toast.error('Failed to add task');
    }
  };

  const handleCompleteTask = async (taskId, dogId) => {
    try {
  ...
  const handleDeleteTask = async (taskId, dogId) => {
    try {
              ...
              <Dialog open={addTaskOpen} onOpenChange={setAddTaskOpen}>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4 mt-4">
                    ...
                    <Button 
                      onClick={() => handleAddTask(selectedDog.dog_id)}
                    ...
                    >
                      <CardContent className="p-4">
                        <div className="flex items-center gap-4">
                          <button
                            onClick={() => !task.is_completed && handleCompleteTask(task.task_id, selectedDog.dog_id)}
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => handleDeleteTask(task.task_id, selectedDog.dog_id)}
